...
#include<math.h>
#include <stdlib.h>
	int IP_Table[64] = {                                     //初始置换IP矩阵
        58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
		62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8,
		57, 49, 41, 33, 25, 17,  9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
		61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7 };
	int E_Table[48] = {                                    //扩展矩阵 选择运算E
...
static  void S(const int input[48],int output[32],int table[8][4][16])//S盒压缩
{
	...
	for(;i<48;i=i+6)
	{
		INT[j]=table[j][(input[i]<<1)+(input[i+5])][(input[i+1]<<3)+(input[i+2]

<<2)+(input[i+3]<<1)+(input[i+4])];
		j++;
	}
	for(j=0;j<8;j++)
	{
...
int mainFuntion2(char MIN[8],char MI[8],int num)
{
    ...
    for(int changeSum=1;changeSum<65;changeSum++){//计算密文位数改变平均值            
     ...
     int randNum =  rand()%64;
     for(int i=0;i<64;i++){tempPlaintextbits[i] = MINbits[i];}//对明文bits重新赋值
     for(int j=0;j<changeSum;j++){//对明文的changeSum位进行改变（改变几位）
     ...
     DES_Efun(tempPlaintext, MI, tempOutput);//获取明文改变后的密文   8位而已
	 ...
	 printf("改变%2d位明文，进行%3d次统计密文平均改变位数为 %f ",changeSum,num,1.0*count/num);
     printf("\n");
	}
    printf("\n*******************************************************\n");
	return 0;
